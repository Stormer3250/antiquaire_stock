...
HEADER_GUESS = {
    ...
    "achat": "achat", "prix": "achat", "stock": "stock", "quantite": "stock",
    "quantité": "stock", "fournisseur": "fournisseur",
}
...
def import_file(name: str, location_id: int, categorie_id: int, create_categories=False):
    ...
    result = call("POST", "/api/import/apply", {
        "token": data["token"], "mapping": mapping, "location_id": location_id,
        "categorie_id": categorie_id, "create_categories": create_categories,
    })
    print(f"{name}: {result.get('created')} créées, {result.get('updated')} mises à jour, "
